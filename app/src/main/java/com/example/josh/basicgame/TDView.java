package com.example.josh.basicgame;
...
public class TDView extends SurfaceView implements Runnable {
    ...
    public void run() {
        ...
        long numFrames = 0;
        long frameMeasurement = 0;
        while (playing) {
            frameMeasurement+= System.currentTimeMillis() - last;
            numFrames++;
            last = System.currentTimeMillis();
            if(frameMeasurement > 999){
                frameMeasurement = 0;
               Log.d("FPS : ",Long.toString(numFrames));
                numFrames = 0;
            }
            update();
            draw();
